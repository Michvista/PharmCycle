...
export default function InventoryPage() {
  ...
  function load() {
    ...
    const apiStatus = statusFilter
      ? statusApiMap[statusFilter] || statusFilter.toUpperCase()
      : undefined;
    Promise.all([
      inventoryApi.list({
      ...
      .then(([listRes, statusRes]) => {
        ...
        const breakdown = Object.fromEntries(
          statusRes.breakdown.map((b) => [b.status, b.count]),
        );
        setStats({
  ...
  return (
    <>
      <TopBar
        title="Inventory"
        subtitle="Manage your medicine stock and track inventory in real-time.">
        <button
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
          <StatCard
            icon={<AppIcon name="package" size={20} className="text-white" />}
            iconBg="bg-green-500"
            title="Total Items"
          ...
          />
          <StatCard
            icon={<AppIcon name="clock" size={20} className="text-white" />}
            iconBg="bg-red-500"
            title="Near Expiry Items"
            value={String(stats.nearExpiry)}
          />
          <StatCard
            icon={<AppIcon name="calendar" size={20} className="text-white" />}
            iconBg="bg-blue-500"
            title="Expired Items"
        ...
        <div className="bg-white rounded-xl border border-gray-100 mb-6">
          <div className="flex flex-wrap items-center gap-3 p-4 border-b border-gray-100">
            ...
            <button
              ...
              className="px-3 py-2 bg-green-600 text-white rounded-xl text-sm cursor-pointer">
              Search
            </button>
          </div>
                ...
                {loading ? (
                  ...
                  items.map((item) => (
                    <tr
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={item.medicine.category as "analgesic"}>
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={inventoryStatusToBadge(item.status)}>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <form
            ...
            <input
              required
              placeholder="NAFDAC Reg. No. (e.g. A4-1234)"
