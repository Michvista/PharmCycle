...
export default function TransferRequestsPage() {
  ...
  const [summary, setSummary] = useState({ total: 0, pending: 0, inTransit: 0, completed: 0, cancelled: 0 });
  ...
  async function handleAction(id: string, action: "accept" | "reject" | "complete") {
    ...
    try {
      ...
      toastSuccess(`Request ${action === "accept" ? "accepted" : action === "reject" ? "rejected" : "completed"}`);
      load();
    } catch (e) {
  ...
  return (
    <>
      <TopBar title="Transfer Requests" subtitle="Manage incoming and outgoing medicine transfer requests." />
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <StatCard icon={<AppIcon name="chart" size={20} className="text-purple-600" />} iconBg="bg-purple-50" title="All Requests" value={String(summary.total)} />
          <StatCard icon={<AppIcon name="clock" size={20} className="text-orange-600" />} iconBg="bg-orange-50" title="Pending" value={String(summary.pending)} />
          <StatCard icon={<AppIcon name="truck" size={20} className="text-blue-600" />} iconBg="bg-blue-50" title="In Transit" value={String(summary.inTransit)} />
          <StatCard icon={<AppIcon name="check" size={20} className="text-green-600" />} iconBg="bg-green-50" title="Completed" value={String(summary.completed)} />
          <StatCard icon={<AppIcon name="cancel" size={20} className="text-red-600" />} iconBg="bg-red-50" title="Cancelled" value={String(summary.cancelled)} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3 bg-white rounded-xl border border-gray-100 p-5">
            {loading ? (
              ...
              <p className="text-sm text-gray-500 py-8 text-center">No {direction} requests</p>
            ) : (
              <div className="space-y-4">
                {requests.map((req) => {
                  const med = req.listing?.inventoryItem?.medicine;
                  const partner = direction === "incoming" ? req.requestingPharmacy : req.listing?.pharmacy;
                  return (
                    <div key={req.id} className="border border-gray-100 rounded-xl p-4">
                      ...
                      {direction === "incoming" && req.status === "PENDING" && (
                        <div className="flex gap-2 mt-3">
                          <button onClick={() => handleAction(req.id, "accept")} disabled={acting === req.id}
                            className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs cursor-pointer disabled:opacity-50">Accept</button>
                          <button onClick={() => handleAction(req.id, "reject")} disabled={acting === req.id}
                            className="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs cursor-pointer disabled:opacity-50">Reject</button>
                        </div>
                      )}
                      {direction === "incoming" && req.status === "ACCEPTED" && (
                        <button onClick={() => handleAction(req.id, "complete")} disabled={acting === req.id}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <DonutChart segments={segments} total={summary.total || 1} centerLabel="Total" size={130} />
