...
export default function TransfersPage() {
  ...
  function load() {
    setLoading(true);
    transfersApi
      .available({ search: search || undefined })
  ...
  async function handleRequestSubmit(e: React.FormEvent) {
    ...
    if (!qty || qty < 1 || qty > requestModal.listing.quantity) {
      toastError(
        `Please enter a quantity between 1 and ${requestModal.listing.quantity}`,
      );
      return;
    }
  ...
  return (
    <>
      <TopBar
        title="Transfers"
        subtitle="Find, request and manage medicine transfers between pharmacies."
      />
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Search available transfer listings from partner pharmacies.
            </p>
            <button
              onClick={load}
              className="w-full py-2 bg-green-600 text-white rounded-lg text-xs font-semibold cursor-pointer">
              Refresh Listings →
            </button>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Click &quot;Request&quot; on any listing below.
            </p>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mb-4">
              List your excess or near-expiry stock.
            </p>
            <button
              onClick={openOfferModal}
              className="w-full py-2 bg-purple-600 text-white rounded-lg text-xs font-semibold cursor-pointer">
              Offer Now →
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <button
              onClick={load}
              className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs cursor-pointer">
              Search
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {listings.map((med) => (
                <div
                  key={med.listingId}
                  className="flex flex-wrap items-center gap-4 p-4 border border-gray-100 rounded-xl hover:bg-gray-50">
                  <div className="flex-1 min-w-[200px]">
                    <h4 className="text-sm font-semibold text-gray-900">
                      {med.medicineName} {med.strength}
                    </h4>
                    <p className="text-xs text-gray-400">
                      {med.dosageForm} · {med.fromPharmacy.name},{" "}
                      {med.fromPharmacy.city}
                    </p>
                  ...
                  <div className="text-right min-w-[120px]">
                    <div className="text-sm font-semibold text-gray-900">
                      {formatNaira(med.discountedPrice)} each
                    </div>
                    {med.discountPercent > 0 ? (
                      <div className="text-xs text-gray-400">
                        Original {formatNaira(med.originalPrice)}
                      </div>
                    ) : (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <form
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {requestModal.listing.medicineName}{" "}
                  {requestModal.listing.strength} from{" "}
                  {requestModal.listing.fromPharmacy.name}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 text-sm text-gray-700">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-gray-900">
                  {formatNaira(requestModal.listing.discountedPrice)}
                </span>
              </div>
              <div className="flex items-center justify-between mt-2">
                <span>Available</span>
                <span className="text-gray-600">
                  {requestModal.listing.quantity} units
                </span>
              </div>
            </div>
            ...
            <input
              ...
              onChange={(e) =>
                setRequestModal({ ...requestModal, quantity: e.target.value })
              }
            ...
            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 text-sm text-gray-700">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-gray-900">
                  {formatNaira(
                    requestModal.listing.discountedPrice *
                      Number(requestModal.quantity || 0),
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setRequestModal(null)}
                className="flex-1 py-2 border rounded-lg text-sm cursor-pointer">
                Cancel
              </button>
              <button
                type="submit"
                disabled={submittingRequest}
                className="flex-1 py-2 bg-green-600 text-white rounded-lg text-sm cursor-pointer disabled:opacity-50">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <form
            ...
            <input
              ...
              onChange={(e) =>
                setOfferForm({ ...offerForm, discountPercent: e.target.value })
              }
              className="w-full px-3 py-2 border rounded-lg text-sm"
            />
            <div className="flex gap-2">
