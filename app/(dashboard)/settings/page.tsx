...
export default function SettingsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setMessage("");
    try {
      await pharmacyApi.updateProfile({ pharmacyName, address, city, state, name, email });
      await refreshMe();
  ...
  return (
    <>
      <TopBar title="Settings" subtitle="Manage your pharmacy profile, notifications, and preferences." />
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <form onSubmit={handleSave} className="max-w-2xl space-y-6">
          {message && (
            <p className={`text-sm px-4 py-3 rounded-xl ${message.includes("success") ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
              {message}
            </p>
          )}

          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <h3 className="text-base font-semibold text-gray-900 mb-4">Pharmacy Profile</h3>
            <div className="space-y-4">
              {[
                { label: "Pharmacy Name", value: pharmacyName, set: setPharmacyName },
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <h3 className="text-base font-semibold text-gray-900 mb-4">Notifications <span className="text-xs text-gray-400 font-normal">(local preferences)</span></h3>
            <div className="space-y-4">
              {Object.entries({ expiry: "Expiry alerts", transfers: "Transfer request notifications", lowStock: "Low stock warnings", aiDigest: "AI insight digests", network: "Network updates" }).map(([key, label]) => (
                <div key={key} className="flex items-center justify-between">
                  <span className="text-sm text-gray-700">{label}</span>
                  <button type="button" onClick={() => setToggles((p) => ({ ...p, [key]: !p[key as keyof typeof p] }))}
                    ...
                    <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${toggles[key as keyof typeof toggles] ? "left-[22px]" : "left-0.5"}`} />
                  </button>
                </div>
              ))}
            </div>
