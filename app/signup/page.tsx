  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (role === "pharmacy") {
        ...
        await registerPharmacy({ pharmacyName, city, state, name, email, password, address: location });
      } else {
        const { city, state } = parseLocation(location || "Lagos, Lagos");
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-white overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          <p className="text-gray-500 mb-8">Get started with PharmCycle today</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {role === "pharmacy" && (
              <div className="mb-4">
                ...
                <input type="text" value={pharmacyName} onChange={(e) => setPharmacyName(e.target.value)} required placeholder="GreenLife Pharmacy"
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
            )}
            ...
            <div className="mb-4">
              ...
              <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Lagos, Lagos"
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>

            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Email address</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="you@pharmacy.com"
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
            ...
            <label className="flex items-start gap-2 mb-6 cursor-pointer">
              <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="w-4 h-4 mt-0.5 accent-green-600" />
